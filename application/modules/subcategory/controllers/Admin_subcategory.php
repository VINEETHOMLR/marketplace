...
class Admin_subcategory extends MX_Controller
{
    ...
    public function __construct()
    {
		...
		$this->load->model( array( 'Subcategory_model' ) );
    }
    ...
    public function list_json( $user_id = NULL )
	{
		...
		$records[ 'rows' ]	=	$this->Subcategory_model->get_list(  $category_status ,  $limit ,  $offset  ,  $date_from ,  $date_to  , $category_search , $user_id,NULL );
		$records[ 'total' ]	=	$this->Subcategory_model->get_count(  $category_status ,  $date_from ,  $date_to  , $category_search , $user_id,NULL);
	...
	{
		$data[ 'content' ] = 'subcategory/form';
		$data[ 'category' ]	=	array();
		if( $category_id != NULL )
		{
			$data[ 'category' ]	=	$this->Subcategory_model->get_category_single( $category_id );
		}
	...
	public function manage()
	{
		...
		$data[ 'content' ] = 'subcategory/table';
		$this->load->view( 'admin/page' , $data );
	}
	...
	public function update( )
	{
		...
		$data[ 'product_category_status' ]	=	$this->input->post( 'product_category_status' ) ? $this->input->post( 'product_category_status' ) : commonStatus::HOLD;
		...
		if( $category_id == NULL )
		{
			...
			$category_id	=	$this->Subcategory_model->add( $data );
		}
		else
		{
			$where[ 'product_category_id' ]	=	$category_id;
			//$data[ 'category_update_date' ]	=	$this->dbnow;
			$this->Subcategory_model->update( $data , $where );
		}
		...
		else
		{
			...
			$msg[ 'red' ]	=	$this->base.'/subcategory/admin_subcategory/manage';
		}
	...
	public function change_status( )
	{
		...
		$where[ 'product_category_id' ]	=	$record_id;
		$data[ $field ]	=	$record_status;
		if( $this->Subcategory_model->update( $data , $where ) )
		{
			$msg['res']		=	1;
	...
	{
		return $this->Subcategory_model->get_list(1);
	}
