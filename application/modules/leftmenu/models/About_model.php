...
class About_model extends MY_Model {
	...
	{
		$this->_table	=	'mv_about';
		$this->primary_key	=	'about_id';
		$this->primary_column	=	'about_title';
	}
    public function get_list(  $about_status = NULL , $limit = NULL , $start = NULL  , $date_from =NULL , $date_to = NULL  )
	{
		/*if(  !Modules::run( 'login/is_admin' ) )
		{
			$this->db->where( 'about.about_status' , CommonStatus::ACTIVE );
		}
		...
		return $this->db->select(" about.*  " )
		->where('about.about_id',1)
		//->order_by('ad.ad_id ASC')
		//->group_by( 'ad.ad_id' )
		->get( $this->_table.' about ')
		->result_array();
	}
