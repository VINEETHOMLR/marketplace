...
/***/ function(module, exports, __webpack_require__) {
	...
	var _mvTableLoader = __webpack_require__(19);

	var _mvTableLoader2 = _interopRequireDefault(_mvTableLoader);
	...
	new _mvTableLoader2.default($table, options);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _seo = __webpack_require__(27);
	...
	var bootstrapTable = __webpack_require__(52);
	var Handlebars = __webpack_require__(54);
	__webpack_require__(67);
	//require('../../js/downloads/x-editable/jquery-editable.css');
	//var editable = require('../../js/downloads/x-editable/jquery-editable-poshytip.min.js');
	//console.log(editable)

	__webpack_require__(68);

	var mvTableLoader = function mvTableLoader(elm, options) {
		var _this = this;

		_classCallCheck(this, mvTableLoader);

		this.init = function () {
			_this.prepareActions();
			_this.prepareQuery();
			_this.bindEvents();
			_this.options.tableConfig['onPostBody'] = _this.bindRowEvents;
			_this.options.tableConfig.queryParamsExtra = _this.options.tableConfig.queryParamsExtra || {};
			_this.table.bootstrapTable(_this.options.tableConfig);
		};

		this.bindEvents = function () {
			if (_this.options.actions.add != undefined) {
				_this.bindAddEvents();
			}

			$('select.js-table-loader-filter').on('change', function (e) {
				var $this = $(e.currentTarget);
				_this.options.tableConfig.queryParamsExtra[$this.data('filter-key')] = $this.val();
				_this.resetTablePagination();
				_this.refreshTable();
			});

			$('input.js-table-loader-search').on('input', function (e) {
				var $this = $(e.currentTarget);
				_this.options.tableConfig.queryParamsExtra[$this.data('search-key')] = $this.val();
				_this.resetTablePagination();
				_this.refreshTable();
			});
			//this.table.bootstrapTable({onLoadSuccess : this.bindRowEvents() });
		};

		this.bindAddEvents = function () {
			if (_this.options.actions.add.url == undefined) {
				var source = $(_this.options.actions.add.template).html();
				if (_this.options.actions.add.hbs != undefined) {
					source = $(_this.options.actions.add.hbs).html();
				}
				_this.formTemplate = Handlebars.compile(source);
				$(_this.options.actions.add.selector).on('click', function (e) {
					e.preventDefault();
					_this.showForm({});
				});
			} else {
				$(_this.options.actions.add.selector).on('click', function (e) {
					e.preventDefault();
					window.location = _this.options.actions.add.url;
				});
			}
		};

		this.bindRowEvents = function (data) {
			$(document).find('.x-editable').each(function (i, e) {
				var $this = $(e);
				var xeditableoptions = $this.data('xeditableoptions');
				xeditableoptions['success'] = function (response, newValue) {
					$this.notify(response.msg, "info");
				};
				$this.editable(xeditableoptions);
			});
			$(document).find('.mv-seo').each(function (i, e) {
				var $this = $(e);
				var seoOptions = $this.data('seo-options');
				new _seo2.default($this, seoOptions);
			});
			//$(document).find('.x-editable').editable();
		};
		...
		this.ajaxCall = function (url, data) {
			$.ajax({
				...
				complete: function complete(data) {
					var msg = data.msg || 'List updated successfully !';
					$.notify(msg, "success");
					_this.refreshTable();
				}
			});
		...
		this.showForm = function (record) {
			var data = { record: record };

			if (typeof formParams != 'undefined') {
				data = $.extend({}, data, formParams);
			}

			var $form = $(_this.formTemplate(data));

			$(_this.options.actions.add.template).html($form);

			var mvFormOptions = {
				ajaxFormOptions: {
					successAfter: function successAfter(responseText, statusText, xhr, $form1) {
						if (responseText.res == 1) {
							$(_this.options.actions.add.modal).modal('hide');
							var msg = responseText.msg || 'List updated successfully !';
							$.notify(msg, "success");
							_this.refreshTable();
						}
					}
			...
			new _mvForm2.default($form, mvFormOptions);
			$(_this.options.actions.add.modal).modal('show');
		};

		this.operateFormatter = function (value, row, index) {

			var actionsH = '<div class="actions">';

			var editOp = ['<a class="edit" href="#" title="Edit" > ', '<i class="glyphicon  glyphicon-edit"></i>', '</a>  '].join('');

			var deleteOp = ['<a class="remove text-danger" href="javascript:void(0)" title="Remove">', '<i class="glyphicon glyphicon-trash"></i>', '</a>'].join('');

			if (_this.options.actions.edit) {
				actionsH = actionsH + editOp;
			}

			if (_this.options.actions.delete) {
				actionsH = actionsH + deleteOp;
			}

			actionsH = actionsH + '</div>';
			return actionsH;
		};

		this.prepareActions = function () {
			if (typeof _this.options.actions == 'undefined' || !Object.keys(_this.options.actions).length) return;
			...
			var operateEvents = {
				'click .remove': function clickRemove(e, value, row, index) {
					...
					var $this = $(e.currentTarget);
					$this.closest('div.actions').html('<i class="fa fa-spinner fa-spin"></i>');
					var deleteUrl = _this.options.actions.delete.url + row[_this.options.tableConfig.uniqueId];
					if (_this.options.actions.delete.red != undefined) {
						window.location = deleteUrl;
					}

					_this.ajaxCall(deleteUrl);
				},

				'click .edit': function clickEdit(e, value, row, index) {
					e.preventDefault();
					if (_this.options.actions.edit.url != undefined) {
						window.location = _this.options.actions.add.url + row[_this.options.tableConfig.uniqueId];
					} else {
						_this.showForm(row);
					}
				}
			...
			_this.options.tableConfig.columns.push(actions);
		};

		this.prepareQuery = function () {
			_this.options.tableConfig.queryParams = function (params) {
				var paramsRefined = $.extend({}, params, _this.options.tableConfig.queryParamsExtra);
				console.log(JSON.stringify(paramsRefined));
				delete _this.options.tableConfig.queryParamsExtra['offset'];
				return paramsRefined;
			};
		};

		this.resetTablePagination = function () {
			_this.options.tableConfig.queryParamsExtra['offset'] = 0;
			_this.options.tableConfig.pageNumber = 1;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var formModalTemplate = __webpack_require__(28);
	var formTemplate = __webpack_require__(48);
	formModalTemplate = $(formModalTemplate({}));

	var seo = function seo(elm, options) {
		var _this = this;
		...
		this.renderButton = function () {
			var seo_add_button = $('<a>', {
				class: _this.record.seo_title ? 'btn btn-block btn-info flat btn-xs' : 'btn bg-maroon btn-flat  btn-block btn-xs',
				href: 'javacsript:void(0)',
				text: _this.record.seo_title ? 'Edit Seo' : 'Add Seo',
				click: _this.showForm
			});
			_this.elm.html(seo_add_button);
			if (_this.record.seo_title) {
				_this.addXEdit();
			}
		};

		this.addXEdit = function () {
			var statusOptions = [{ value: 1, text: 'Active' }, { value: 2, text: 'On Hold' }];
			var xEditableOptions = {
				value: _this.record.seo_status,
				defaultValue: "Not active",
				type: "select",
				pk: _this.record.seo_id,
				name: 'seo_status',
			...
			console.log(xEditableOptions);
			frmt.editable(xEditableOptions);
			_this.elm.append(frmt);
		};

		this.showForm = function () {
			_this.form = $(formTemplate({ record: _this.record }));
			var mvFormOptions = {
				ajaxFormOptions: {
					successAfter: function successAfter(responseText, statusText, xhr, $form1) {
						formModalTemplate.modal('hide');
						$.notify("Seo updated", 'success');
						_this.record.seo_title = responseText.record.seo_title;
						_this.record.seo_keywords = responseText.record.seo_keywords;
						_this.record.seo_description = responseText.record.seo_description;
						_this.renderButton();
					}
				}

			};
			new _mvForm2.default(_this.form, mvFormOptions);
			formModalTemplate.find('.modal-body').html(_this.form);
			formModalTemplate.modal('show');
		};

		this.elm = elm;
		this.form;
		this.record = {
			content_type: options.record.content_type,
			seo_id: options.record.seo_id || false,
			seo_status: options.record.seo_status || 1,
			content_ref_id: options.record.content_ref_id,
			seo_title: options.record.seo_title,
			seo_keywords: options.record.seo_keywords,
			seo_description: options.record.seo_description
		};
		this.init();
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<form class=\"mvform\" method=\"post\" action=\""
	    + alias1(__default(__webpack_require__(49)).call(depth0 != null ? depth0 : {},{"name":"base","hash":{},"data":data}))
	    + "/seo/update\" >\r\n <input type=\"hidden\" name=\"seo_id\" value=\""
	    + alias1(alias2(((stack1 = (depth0 != null ? depth0.record : depth0)) != null ? stack1.seo_id : stack1), depth0))
	    + "\">\r\n <input type=\"hidden\" name=\"content_type\" value=\""
	    + alias1(alias2(((stack1 = (depth0 != null ? depth0.record : depth0)) != null ? stack1.content_type : stack1), depth0))
	    + "\">\r\n <input type=\"hidden\" name=\"content_ref_id\" value=\""
	    + alias1(alias2(((stack1 = (depth0 != null ? depth0.record : depth0)) != null ? stack1.content_ref_id : stack1), depth0))
	    + "\">\r\n  <div class=\"row form-group\">\r\n    <div class=\"col-md-4 \">Page Title<span class=\"text-danger\">*</span> </div>\r\n    <div class=\"col-md-8\">\r\n      <input type=\"text\" name=\"seo_title\"  value=\""
	    + alias1(alias2(((stack1 = (depth0 != null ? depth0.record : depth0)) != null ? stack1.seo_title : stack1), depth0))
	    + "\" class=\"form-control\"  placeholder=\"Enter Page Title\"  required>\r\n    </div>\r\n  </div>\r\n  <div class=\"row form-group\">\r\n    <div class=\"col-md-4 \">Page Description<span class=\"text-danger\">*</span> </div>\r\n    <div class=\"col-md-8\">\r\n      <textarea name=\"seo_description\" class=\"form-control\"  placeholder=\"Enter Page Description\"  required>"
	    + alias1(alias2(((stack1 = (depth0 != null ? depth0.record : depth0)) != null ? stack1.seo_description : stack1), depth0))
	    + "</textarea>\r\n    </div>\r\n  </div>\r\n   <div class=\"row form-group\">\r\n    <div class=\"col-md-4 \">Page Keywords<span class=\"text-danger\">*</span> </div>\r\n    <div class=\"col-md-8\">\r\n      <textarea name=\"seo_keywords\" class=\"form-control\"  placeholder=\"Enter Page Keywords\"  required >"
	    + alias1(alias2(((stack1 = (depth0 != null ? depth0.record : depth0)) != null ? stack1.seo_keywords : stack1), depth0))
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(false) {
		...
		if(!content.locals) {
			module.hot.accept("!!./../../../scripts/node_modules/raw-loader/index.js!!./bootstrap-table.min.css", function() {
				var newContent = require("!!./../../../scripts/node_modules/raw-loader/index.js!!./bootstrap-table.min.css");
...
/***/ function(module, exports, __webpack_require__) {
	...
	var handlebars = __webpack_require__(55)['default'];

	var printer = __webpack_require__(65);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _handlebarsCompilerAst = __webpack_require__(56);

	var _handlebarsCompilerAst2 = _interopRequireDefault(_handlebarsCompilerAst);

	var _handlebarsCompilerBase = __webpack_require__(57);

	var _handlebarsCompilerCompiler = __webpack_require__(62);

	var _handlebarsCompilerJavascriptCompiler = __webpack_require__(63);

	var _handlebarsCompilerJavascriptCompiler2 = _interopRequireDefault(_handlebarsCompilerJavascriptCompiler);

	var _handlebarsCompilerVisitor = __webpack_require__(60);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _whitespaceControl = __webpack_require__(59);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _codeGen = __webpack_require__(64);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _handlebars = __webpack_require__(54);

	var _handlebars2 = _interopRequireDefault(_handlebars);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	_handlebars2.default.registerHelper('logOp', function (v1, operator, v2, options) {
		switch (operator) {
			case '==':
	...
	_handlebars2.default.registerHelper('select', function (options, name, selected) {
		var element = $('<select/>', {
			name: name,
			class: 'form-control'
		});
		...
		$.each(options, function (key, value) {
			var slctd = (typeof selected === 'undefined' ? 'undefined' : _typeof(selected)) != undefined && selected == key ? true : false;
			element.append($("<option></option>", {
				'value': key,
				'text': value
			}).attr('selected', slctd));
		...
		return new _handlebars2.default.SafeString(element.prop('outerHTML'));
	});

	_handlebars2.default.registerHelper('base', function () {
		return base;
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	__webpack_require__(2)(__webpack_require__(69))
